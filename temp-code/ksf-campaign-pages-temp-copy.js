    ...
    success: function (result) {
      if (result.code != 200) {
        ...
        const onFailed = function(failReason) {
        	$("#divLoading").hide();
    	 		$("#one-pager-failed").css({"background-color": "#ffbaba", "color": "black", "padding": "8px 16px", "margin-bottom": "16px"}); 
          if (failReason == 'SubscriptionExistsError') {
      	 		// $("#subscription-exists-error").show()
      	 		// $("#subscription-exists-error").html('Eftersom du redan har en prenumeration är detta erbjudande inte tillgängligt för dig.')
